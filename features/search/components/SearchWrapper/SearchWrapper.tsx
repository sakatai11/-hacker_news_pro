              ...
              <Link href={article.url} >
                <Card sx={{ display: 'flex' }}>
                  <Box sx={{
                    ...
                    <CardMedia
                      component="img"
                      image={article.urlToImage ? article.urlToImage : dummy.src}
                      width={320}
                      height={160}
                      alt="アイキャッチ"
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: "space-between", padding: 2,  flex: { xs: 1, sm: "initial" },}}>
                    <Box>
                      <Typography variant="subtitle1" color="text.secondary" component="div" fontSize="12px">
                        {new Date(article.publishedAt).toLocaleDateString('ja-JP', options)}
                      </Typography>
